...
import type { HeatmapChartProps, HeatmapMemoResult } from "../types/components";
...
export default function HeatmapChart({ data = [] }: HeatmapChartProps) {
  const { cols, max } = useMemo<HeatmapMemoResult>((): HeatmapMemoResult => {
    if (!data.length) return { cols: [], max: 0 };
    const max = Math.max(...data.map((d: HeatmapItem) => d.count));
    const cols: (HeatmapItem | null)[][] = [];
    let col: (HeatmapItem | null)[] = [];

    data.forEach((d: HeatmapItem, i: number) => {
  ...
  const totalCount = data.reduce((s: number, d: HeatmapItem) => s + d.count, 0);
