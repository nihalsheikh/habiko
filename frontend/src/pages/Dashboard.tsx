...
import TodayHabitCard from "../components/TodayHabitCard.js";
import WeeklyGrid from "../components/WeeklyGrid.js";
import HeatmapChart from "../components/HeatmapChart.js";
import SummaryCards from "../components/SummaryCards.js";
import AIWeeklyReport from "../components/AIWeeklyReport.js";
import MorningMotivation from "../components/MorningMotivation.js";
import HabitSuggestionModal from "../components/HabitSuggestionModal.js";
import StreakRecoveryCard from "../components/StreakRecoveryCard.js";
...
import type { Habit, HabitLog, HeatmapItem } from "../types/api.js";
import type {
  HabitFormSubmitData,
  HabitSuggestion,
} from "../types/components.js";
import type { LogsByHabitMap, StreaksMap } from "../types/pages.js";
...
export default function Dashboard() {
  ...
  const [allLogsByHabit, setAllLogsByHabit] = useState<LogsByHabitMap>({});
  ...
  const loadAll = async (): Promise<void> => {
    ...
    try {
      ...
      const [habitsRes, todayRes, rangeRes, heatRes] = await Promise.all([
        ...
        api.get<HeatmapItem[]>("/logs/heatmap"),
      ]);
  ...
  const completedToday = useMemo(
    () => new Set<string>(todayLogs.map((l) => String(l.habitId))),
    [todayLogs],
  );

  const weekLogsByHabit = useMemo<LogsByHabitMap>(() => {
    const out: LogsByHabitMap = {};
  ...
  const streaksById = useMemo<StreaksMap>(() => {
    const out: StreaksMap = {};
  ...
  const toggle = async (habit: Habit): Promise<void> => {
  ...
  const saveHabit = async (data: HabitFormSubmitData): Promise<void> => {
    setSubmitting(true);
    try {
      if (editing) {
  ...
  const deleteHabit = async (habit: Habit): Promise<void> => {
  ...
  const archiveHabit = async (habit: Habit): Promise<void> => {
  ...
  const acceptSuggestion = async (s: HabitSuggestion): Promise<void> => {
      ...
      >
        ...
        <div className="flex justify-end gap-2 mt-5">
          ...
          <button
            ...
            onClick={() => deleteTarget && deleteHabit(deleteTarget)}
          >
            Delete
          </button>
        </div>
