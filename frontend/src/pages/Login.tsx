...
import type { LocationStateWithFrom } from "../types/pages";
...
export default function Login() {
  ...
  const submit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      const state = loc.state as LocationStateWithFrom | null;
      navigate(state?.from || "/dashboard", { replace: true });
    } catch (error) {
      const axiosErr = error as AxiosError<{ message?: string }>;
      setErr(axiosErr.response?.data?.message || "Login failed");
    } finally {
      setLoading(false);
    }
  };
