...
const REPORT_CACHE_KEY = (weekStart: string): string =>
...
export default function Insights() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (cached) {
          try {
            const { content, generatedAt }: CachedWeeklyReport =
              JSON.parse(cached);
            setReport(content);
  ...
  const dailyData: DailyChartData[] = thisWeek.map((d) => {
    const count = thisWeekLogs.filter((l) => l.completedDate === d.key).length;
    return { label: d.label, count };
  });

  const compareData: CompareChartData[] = thisWeek.map((d, idx) => {
  ...
  const perHabit: PerHabitPerformance[] = useMemo(() => {
    return habits
