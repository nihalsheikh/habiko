...
import type { WeekDayKey } from "../types/components";
import type { DayTotalItem, PerHabitDoneCount } from "../types/pages";
...
export default function Weekly() {
  ...
  const days: WeekDayKey[] = useMemo(() => weekKeysFor(cursor), [cursor]);
  ...
  const logsByHabit = useMemo<Record<string, string[]>>(() => {
  ...
  const dayTotals: DayTotalItem[] = days.map((d) => ({
    ...d,
    count: logs.filter((l) => l.completedDate === d.key).length,
  }));
  const bestDay: DayTotalItem | undefined = [...dayTotals].sort(
    (a, b) => b.count - a.count,
  )[0];

  const perHabitDone: PerHabitDoneCount[] = habits
    .map((h) => ({
      h,
      count: (logsByHabit[h._id] || []).length,
    }))
    .sort((a, b) => b.count - a.count);
  const topHabit: PerHabitDoneCount | undefined = perHabitDone[0];
