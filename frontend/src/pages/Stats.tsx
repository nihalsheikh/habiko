...
export default function Stats() {
  ...
  const monthly = useMemo<MonthlyStatItem[]>(() => {
    const end = new Date();
  ...
  const weekly = useMemo<WeeklyStatItem[]>(() => {
    const end = new Date();
    const out: WeeklyStatItem[] = [];
    for (let i = 6; i >= 0; i--) {
      const d = subDays(end, i);
      const key = format(d, "yyyy-MM-dd");
  ...
  const best: HabitStatsItem | undefined = sortedByStreak[0];
  const sortedByComp = [...stats.perHabit].sort(
    (a, b) => b.completions30d - a.completions30d,
  );
  const longestLongest: HabitStatsItem | undefined = [...stats.perHabit].sort(
    (a, b) => b.longestStreak - a.longestStreak,
  )[0];
  const worst: HabitStatsItem | undefined = [...stats.perHabit]
