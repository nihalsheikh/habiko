...
import { mockUser, mockHabits, mockLogs, mockAI } from "../utils/mockData.ts";
import type {
  User,
  Habit,
  HabitLog,
...
const state: MockState = {
  ...
  habits: mockHabits.map((h: Habit) => ({ ...h })),
  logs: mockLogs.map((l: HabitLog) => ({ ...l })),
};

const delay = (ms: number = 250): Promise<void> =>
  new Promise((r) => setTimeout(r, ms));

const todayKey = (): string => format(new Date(), "yyyy-MM-dd");

const mockStreak = (keys: string[]): StreakResult => {
